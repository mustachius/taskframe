...
func (s *Store) Import(d *Dump) error {
	...
	for _, t := range d.Tasks {
		if _, err := tx.Exec(`INSERT INTO tasks
			(id, parent_id, title, project, priority, status, due, wait, scheduled, recur, created_at, modified_at, completed_at)
			VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
			...
			fmtTime(t.CreatedAt), fmtTime(t.ModifiedAt), fmtTimePtr(t.CompletedAt)); err != nil {
